...
- **Always use the `topic-folder-manager` skill** to resolve a topic to a folder path. Do not classify topics ad-hoc.
...
When in doubt about classification, default to `backend/`. When in doubt about a fact, search the web — this is a learning repo, getting things subtly wrong defeats the entire point.
